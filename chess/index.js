...
document.addEventListener("DOMContentLoaded", () => {
  ...
  squares.forEach((square, i) => {
    ...
    square.style.backgroundColor = (row % 2 === 0) === (i % 2 === 0) ? "#729153" : "#e7e7cb";
    if (square.innerHTML !== "") {
        ...
        case "pawn":
          ...
          if (startPieceColor === "rgb(255, 255, 255)") {
            ...
            validMove = (s2 === s1 + 8 && !endPiece) || 
                        (s2 === s1 + 16 && s1 < 16 && !endPiece && !document.getElementById(s1 + 8).querySelector(".pieces")) || 
                        (endPiece && endPieceColor !== startPieceColor && (s2 === s1 + 7 || s2 === s1 + 9));
            if (validMove && Math.floor(s2 / 8) === 7) {
              // Promotion
              startSquare.innerHTML = "";
              endSquare.innerHTML = queen;
              endSquare.querySelector(".pieces").style.fill = "#fff";
              endSquare.setAttribute("draggable", "true");
              whiteTurn = !whiteTurn;
              const turn = document.querySelector(".turn");
              turn.innerHTML = whiteTurn ? "It's white's turn" : "It's black's turn";
              if (endPiece && endPiece.id === "king") {
            ...
            validMove = (s2 === s1 - 8 && !endPiece) || 
                        (s2 === s1 - 16 && s1 >= 48 && !endPiece && !document.getElementById(s1 - 8).querySelector(".pieces")) || 
                        (endPiece && endPieceColor !== startPieceColor && (s2 === s1 - 7 || s2 === s1 - 9));
            if (validMove && Math.floor(s2 / 8) === 0) {
              // Promotion
              startSquare.innerHTML = "";
              endSquare.innerHTML = queen;
              endSquare.querySelector(".pieces").style.fill = "#372023";
              endSquare.setAttribute("draggable", "true");
              whiteTurn = !whiteTurn;
              const turn = document.querySelector(".turn");
              turn.innerHTML = whiteTurn ? "It's white's turn" : "It's black's turn";
